function [row, col, rad] = blob_detect_inefficient(img, sigma_init, numOfScales, k)
...
% increase filter size and keep image size same
 ...
 for s = 1:numOfScales
     ...
     sigma_new = sigma_init * k^(s-1);
     filtSize = 2*ceil(3*sigma_new) + 1;
     LoG = sigma_new^2 * fspecial('log', filtSize, sigma_new);
     imgFilt = imfilter(img, LoG, 'same', 'replicate');
     imgFilt = imgFilt .^ 2;
     scale_space(:, :, s) = imgFilt;
